...
export function applyInteraction(particle, mouseX, mouseY, interactionMode, interactionStrength, interactionRadius) {
  ...
  switch (interactionMode) {
    ...
    case 'dimensionalRift':
      // Shift particles between dimensions
      if (!particle.inAlternateDimension && Math.random() < 0.03 * force * interactionStrength) {
        particle.inAlternateDimension = true;
        particle.dimensionShiftTime = Date.now();
        particle.originalRadius = particle.radius;
        particle.originalHasGravity = particle.hasGravity;
        particle.originalColor = particle.color;
        particle.z = 0; // Add a z-coordinate for 3D effect
      } else if (particle.inAlternateDimension && 
                Math.random() < 0.02 * force * interactionStrength) {
        particle.inAlternateDimension = false;
        particle.z = 0;
        particle.hasGravity = particle.originalHasGravity;
      }
      
      // Apply dimensional effects if in alternate dimension
      if (particle.inAlternateDimension) {
        const timeFactor = (Date.now() - particle.dimensionShiftTime) * 0.001;
        particle.z = Math.sin(timeFactor) * 20 * interactionStrength;
        particle.radius = particle.originalRadius * (0.5 + Math.sin(timeFactor * 3) * 0.3);
        particle.hasGravity = !particle.originalHasGravity;
        particle.color = `hsla(${(timeFactor * 50) % 360}, 100%, 50%, ${0.5 + Math.sin(timeFactor) * 0.5})`;
        
        // Occasional teleportation within the field
        if (Math.random() < 0.01 * interactionStrength) {
          const angle = Math.random() * Math.PI * 2;
          const jumpDistance = Math.random() * interactionRadius * 0.5;
          particle.x = mouseX + Math.cos(angle) * jumpDistance;
          particle.y = mouseY + Math.sin(angle) * jumpDistance;
        }
      }
      particle.highlighted = true;
      break;

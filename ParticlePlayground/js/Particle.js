...
export class Particle {
  constructor(x, y, canvas, type = 'plasma') {
    ...
    // Dimensional rift properties
    this.inAlternateDimension = false;
    this.dimensionShiftTime = 0;
    this.z = 0; // For 3D effect when in dimensional rift
    this.originalRadius = this.radius;
    this.originalHasGravity = this.hasGravity;
    this.originalColor = this.color;
  }

  setupParticleProperties() {
    // We'll preserve gravity and other physics properties even when changing types
    const hadGravity = this.hasGravity !== undefined ? this.hasGravity : true;
    ...
    this.hasGravity = hadGravity;
    this.trailEnabled = this.trailEnabled || false;
    this.trail = this.trail || [];
    this.maxTrailLength = this.maxTrailLength || 20;
    this.collisionEffect = this.collisionEffect || 'none';
    this.customColor = this.customColor || null;
    this.opacity = 1;
    
    setupParticlePropertiesByType(this);
    
    // After type setup, restore gravity setting
    this.hasGravity = hadGravity;
  }

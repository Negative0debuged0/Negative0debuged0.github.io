...
export function drawParticle(ctx, particle) {
  ...
  // Apply special rendering for dimensional rifts
  if (particle.inAlternateDimension) {
    drawDimensionalParticle(ctx, particle, safeRadius, finalOpacity);
  }
  // Apply special rendering for certain particle types
  else if (particle.type === 'quantum' && Math.random() < 0.3) {
...
function drawDimensionalParticle(ctx, particle, radius, opacity) {
  // Added safety checks for all values
  if (!isFinite(particle.x) || !isFinite(particle.y) || !isFinite(radius) || radius <= 0) {
    // Draw fallback simple particle if coordinates are invalid
    ctx.beginPath();
    ctx.arc(isFinite(particle.x) ? particle.x : 0, 
            isFinite(particle.y) ? particle.y : 0, 
            isFinite(radius) ? radius : 5, 0, Math.PI * 2);
    ctx.fillStyle = `hsla(180, 100%, 50%, ${opacity})`;
    ctx.fill();
    return;
  }
  
  const zScale = Math.max(0.5, 1 + (isFinite(particle.z) ? particle.z / 50 : 0)); // Scale based on z position
  const scaledRadius = radius * zScale;
  
  // Draw particle shadow based on "height" in z-axis
  ctx.beginPath();
  ctx.arc(particle.x, particle.y, scaledRadius * 0.8, 0, Math.PI * 2);
  ...
  // Create a flickering/warping effect
  const warpFactor = Math.sin(Date.now() * 0.01) * 0.2;
  const segments = 6;
  
  for (let i = 0; i <= segments; i++) {
    const angle = (i / segments) * Math.PI * 2;
    const distortion = 1 + (Math.sin(angle * 3 + Date.now() * 0.005) * warpFactor);
    const x = particle.x + Math.cos(angle) * scaledRadius * distortion;
    const y = particle.y + Math.sin(angle) * scaledRadius * distortion;
  ...
  // Use a simple fill color instead of a gradient for more stability
  ctx.fillStyle = `hsla(${(Date.now() * 0.05) % 360}, 100%, 50%, ${opacity})`;
  ctx.fill();
  
  // Add a glowing rim effect
  ctx.strokeStyle = `hsla(${(Date.now() * 0.05) % 360}, 100%, 70%, ${opacity * 0.8})`;
  ctx.lineWidth = 1.5;
  ctx.stroke();
}
